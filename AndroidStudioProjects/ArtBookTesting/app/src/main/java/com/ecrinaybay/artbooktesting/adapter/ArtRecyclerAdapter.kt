package com.ecrinaybay.artbooktesting.adapter
...
class ArtRecyclerAdapter @Inject constructor(
    val glide : RequestManager
): RecyclerView.Adapter<ArtRecyclerAdapter.ArtViewHodler>() {

    class ArtViewHodler(itemView : View) : RecyclerView.ViewHolder(itemView)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ArtViewHodler {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.art_row,parent,false)
        return ArtViewHodler(view)
    }
    ...
    override fun onBindViewHolder(holder: ArtViewHodler, position: Int) {
        val imageView = holder.itemView.findViewById<ImageView>(R.id.artRowImageView)
        val nameText = holder.itemView.findViewById<TextView>(R.id.artRowNameText)
        val artistNameText = holder.itemView.findViewById<TextView>(R.id.artRowArtistNameText)
        val yearText = holder.itemView.findViewById<TextView>(R.id.yearText)
